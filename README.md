...
项目使用了两种机器学习算法进行黄金价格涨跌预测：

1. **逻辑回归 (Logistic Regression)**
   - 线性分类模型，适合二分类问题
   - 训练参数：`max_iter=1000`, `random_state=42`

2. **随机森林 (Random Forest)**
   - 集成学习模型，由多个决策树组成
   - 训练参数：`n_estimators=200`, `random_state=42`, `class_weight="balanced"`

### 算法优势

#### 逻辑回归优势
- **简单高效**：训练速度快，计算资源消耗低
- **可解释性强**：能够提供特征重要性，解释预测结果
- **适合线性关系**：对线性特征组合建模效果好
- **概率输出**：直接输出涨跌概率，便于风险评估

#### 随机森林优势
- **非线性建模**：能够捕捉特征间的非线性关系
- **鲁棒性强**：对异常值和噪声不敏感
- **不容易过拟合**：通过集成多个决策树减少过拟合风险
- **特征重要性**：能够评估各个特征对预测的贡献
- **处理高维特征**：适合处理多种技术指标组成的高维特征空间

### 模型选择策略

项目采用滚动训练与评估方法，对每个预测周期（1/3/5天）：
1. 使用 Walk Forward 方法进行模型评估
2. 计算每个模型的准确率和 AUC 指标
3. 选择表现最佳的模型用于预测
4. 保存最佳模型及评估指标

这种策略能够确保模型适应市场变化，提高预测的可靠性。
